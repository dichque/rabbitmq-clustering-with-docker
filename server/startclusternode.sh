...
sudo /usr/sbin/rabbitmqctl add_vhost $curhostname >> $logfile

echo "Running: rabbitmqctl set_permissions -p $curhostname guest '.*' '.*' '.*'" >> $logfile
sudo /usr/sbin/rabbitmqctl set_permissions -p $curhostname guest ".*" ".*" ".*"  >> $logfile
sleep 5
...
if [ -f $firsttimefile ]; then
  ...
  sudo /usr/sbin/rabbitmq-server -d &
  echo "Waiting for RabbitMQ Server to start" >> $logfile
  sleep 3
  echo "Enabling Plugins" >> $logfile
  sudo /usr/sbin/rabbitmq-plugins enable rabbitmq_mqtt rabbitmq_stomp rabbitmq_management  rabbitmq_management_agent rabbitmq_management_visualiser rabbitmq_federation rabbitmq_federation_management sockjs >> $logfile
  ...
  sudo kill -9 `ps auwwx | grep rabbitmq-server | awk '{print $2}'`
  sleep 1
...
if [ -z "$CLUSTERED" ]; then
  ...
  sudo /usr/sbin/rabbitmq-server  >> $logfile
  ...
  if [ -z "$CLUSTER_WITH" ]; then
    ...
    sudo /usr/sbin/rabbitmq-server -detached >> $logfile

    echo "Stopping App with /usr/sbin/rabbitmqctl stop_app" >> $logfile
    sudo /usr/sbin/rabbitmqctl stop_app >> $logfile
    ...
    if [ -z "$RAM_NODE" ]; then
      ...
      sudo /usr/sbin/rabbitmqctl join_cluster rabbit@$CLUSTER_WITH >> $logfile
    else
      echo "Attempting to join as RAM node: /usr/sbin/rabbitmqctl join_cluster --ram rabbit@$CLUSTER_WITH" >> $logfile
      sudo /usr/sbin/rabbitmqctl join_cluster --ram rabbit@$CLUSTER_WITH >> $logfile
    fi
    echo "Starting App" >> $logfile
    sudo /usr/sbin/rabbitmqctl start_app >> $logfile
